...
def timegan(ori_data, parameters, min_val, max_val):
    ...
    def train_step_discriminator(X_mb, Z_mb):
        ...
        with tf.GradientTape() as disc_tape:
            ...
            #Gives gradient of None???
            gp = gradient_penalty(discriminator_model, H_mb, H_hat_mb)
            disc_loss = gp * gp_weight + disc_loss

            # Train discriminator (only when the discriminator does not work well)
            # if (disc_loss > 0.15): (temporarily comment this out!!!)
            #    disc_loss = get_discriminator_loss(Y_real_mb, Y_fake_mb, Y_fake_e_mb)

        # does not work inside tf function
        # weights = discriminator_model.get_weights()
        # weights = [np.clip(w, -0.01, 0.01) for w in weights]
        # discriminator_model.set_weights(weights)
    ...
    def train():
        ...
        for itt in range(iterations):
            ...
            X_mb = tf.keras.preprocessing.sequence.pad_sequences(X_mb, maxlen=max_seq_len, dtype='float64', value=-1)
            # Train embedder
        ...
        for itt in range(iterations):
            ...
            # Set mini-batch
            X_mb = batch_generator(ori_data, batch_size) 
            X_mb = tf.keras.preprocessing.sequence.pad_sequences(X_mb, maxlen=max_seq_len, dtype='float64')
            # Random vector generation 
            Z_mb = random_generator(batch_size, z_dim, max_seq_len)
            #train discriminator
            d_loss = train_step_discriminator(X_mb, Z_mb)

            # weight clippings (not sure if clipping it here is correct)
            # weights = discriminator_model.get_weights()
            # weights = [np.clip(w, -0.01, 0.01) for w in weights]
            # discriminator_model.set_weights(weights)
        ...
        generated_data = recovery_model(H_hat_generated)

        generated_data = generated_data * (np.array(max_val) - np.array(min_val))
        generated_data = generated_data + np.array(min_val)
...
from data_loading import real_data_loading, sine_data_generation, real_data_loading_prism_dt

data_name = 'prism_dt'
seq_len = 50

if data_name in ['stock', 'energy', 'sine_sampling']:
 ori_data = real_data_loading(data_name, seq_len)
elif data_name == "prism_dt":
    ori_data, min_val, max_val = real_data_loading_prism_dt()
elif data_name == 'sine':
...
generated_data = timegan(ori_data, parameters, min_val, max_val)
